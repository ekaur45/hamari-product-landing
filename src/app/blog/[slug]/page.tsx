...
import { BlogPosting } from "@/types/ldjson.type";
import { Metadata } from "next";
import Link from "next/link";


const getStaticProps = async ({ params }: { params: Promise<{ slug: string }> }) => {
...
export default async function BlogDetailsPage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    const jsonLd:BlogPosting = {
        ...
        headline: blog.title,
        description: blog.excerpt,
        image: process.env.NEXT_PUBLIC_CMS_ASSETS_URL + blog.coverImage?.url,
        author:{
            "@type": "Person",
            name: blog.author?.name,
            url: `http://localhost:3000/team/${blog.author?.slug}`,
            sameAs: `http://localhost:3000/team/${blog.author?.slug}`,
        },
        publisher:{
            "@type": "Organization",
            name: "Taleemiyat",
        ...
        mainEntityOfPage: `http://localhost:3000/blog/${blog.slug}`,
