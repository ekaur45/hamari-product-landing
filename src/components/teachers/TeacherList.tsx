"use client";
import { ApiResponse, PaginatedApiResponse } from "@/services/api.service";
...
export default function TeacherList({ teachersData, subjectsData }: { teachersData: PaginatedApiResponse<Teacher>, subjectsData: ApiResponse<Subject[]> }) {
    ...
    const getTeachers = async (filter: URLSearchParams) => {
        const response: PaginatedApiResponse<Teacher> = await teacherService.getTeachers(filter);
        setSearchedTeachers(response);
    }
    return (
        <div>
            <TeacherFilters subjects={subjectsData?.data || []} onSearch={onFilterChange} />
                ...
                <div className="flex flex-wrap -mx-4">
                    <div className="w-full p-4 md:p-0">
                        ...
                        <div className="flex flex-col sm:flex-row justify-between md:items-end sm:items-center mb-4 md:mb-8 gap-4 px-2">
                            <div className="flex flex-col gap-1">
                                <h1 className="text-lg md:text-3xl font-black text-gray-900 font-outfit">
                                    {searchedTeachers.pagination?.total || 0} Teachers available
                                </h1>
                        ...
                        <div className="space-y-6">
                            {searchedTeachers?.data?.map((teacher: Teacher) => (
                                <TeacherCard key={teacher.id} teacher={teacher} />
                            ))}

                            {(!searchedTeachers.data || searchedTeachers.data.length === 0) && (
                        ...
                        {/* Pagination */}
                        {searchedTeachers.pagination?.totalPages > 1 && (
                            <div className="mt-16 flex items-center justify-center gap-3">
                                ...
                                {[...Array(Math.min(5, searchedTeachers.pagination.totalPages))].map((_, i) => (
                                    <button
                                        key={i}
